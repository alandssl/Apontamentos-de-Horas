...
import { FileBarChart, Search, Building, Clock, Briefcase, User, CalendarDays, Download } from "lucide-react";
import {
    Select,
    SelectContent,
    SelectItem,
...
export default function AdminReportsPage() {
        ...
        fetch("http://localhost:8080/horas")
            ...
            .then(data => {
                const fetchedEntries = data
                    ...
                    .map((h: any) => ({
                        id: String(h.id),
                        date: new Date(h.dataApontamentoId?.data || new Date()).toISOString().split('T')[0], // yyyy-mm-dd
    ...
    const filteredReports = reports.filter((report) => {
        ...
        // Filtro de Nome
        if (searchName && !report.userName.toLowerCase().includes(searchName.toLowerCase())) {
            return false;
        }
    ...
    const exportToExcel = () => {
        ...
        const headers = ["Nome", "Data", "Empresa (CIF)", "Tipo", "Horas", "Status"];
        ...
        link.setAttribute("download", `relatorio_apontamentos_${format(new Date(), "yyyyMMdd_HHmm")}.csv`);
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="bg-zinc-50 dark:bg-zinc-900/50 p-4 rounded-xl border grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select value={month} onValueChange={setMonth} disabled={!!specificDate}>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select value={year} onValueChange={setYear} disabled={!!specificDate}>
                                <SelectTrigger className="h-9 text-sm bg-white dark:bg-zinc-950">
                    ...
                    <div>
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="font-semibold text-lg">Resultados ({filteredReports.length})</h3>
                        </div>
                        ...
                        {filteredReports.length === 0 ? (
                            <div className="text-center py-12 text-muted-foreground border-2 border-dashed rounded-lg bg-zinc-50/50 dark:bg-zinc-900/50 flex flex-col items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-zinc-900 dark:text-zinc-100">Nenhum apontamento encontrado</h3>
                                    <p className="text-sm">Os filtros atuais não retornaram dados aprovados.</p>
                                </div>
                            </div>
                        ) : (
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <Badge variant="secondary" className="bg-green-100/50 text-green-700 dark:bg-green-900/20 dark:text-green-400 font-normal px-1.5 py-0 mt-1">
                                                    Aprovado
                                                </Badge>
                                            </div>
                                        </div>
